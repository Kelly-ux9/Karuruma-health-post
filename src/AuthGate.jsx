  ...
  async function register(e) {
    ...
    try {
      if (!name.trim() || !username.trim() || !email.trim() || password.length < 8) {
        throw new Error("Enter your full name, username, valid email, and a password of at least 8 characters.");
      }
      const { data: existingUsername } = await supabase.rpc("khp_get_login_email", { p_username: username.trim() });
      if (existingUsername) throw new Error("That username is already in use. Please choose another username.");
      const { data: existingEmail } = await supabase.from("users").select("id").ilike("email", email.trim()).maybeSingle();
      if (existingEmail) throw new Error("That email is already registered. Use Forgot password or choose another email.");
      ...
      if (data.session) setMessage("Account created successfully. You are now signed in.");
      else { setMessage("Account created successfully. Check your email and confirm your account before signing in."); setMode("login"); }
  ...
  return <div className="min-h-screen bg-gradient-to-br from-teal-800 via-teal-700 to-emerald-700 flex items-center justify-center p-4" style={{ fontFamily: "system-ui, sans-serif" }}>
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-7">
        ...
        {mode === "register" && <form onSubmit={register}><p className="text-sm text-slate-500 mb-5">Create a new staff account using secure Supabase Authentication.</p><label className="block text-sm font-medium text-slate-600 mb-4">Full name<input className={input + " mt-1.5"} value={name} onChange={e => setName(e.target.value)} autoFocus required /></label><label className="block text-sm font-medium text-slate-600 mb-4">Username<input className={input + " mt-1.5"} value={username} onChange={e => setUsername(e.target.value)} required /></label><label className="block text-sm font-medium text-slate-600 mb-4">Email<input type="email" className={input + " mt-1.5"} value={email} onChange={e => setEmail(e.target.value)} required /></label><label className="block text-sm font-medium text-slate-600 mb-5">Password<input type="password" minLength={8} className={input + " mt-1.5"} value={password} onChange={e => setPassword(e.target.value)} required /><span className="text-xs text-slate-400 mt-1 block">At least 8 characters.</span></label><button disabled={loading} className="w-full py-2.5 rounded-xl bg-teal-700 text-white font-bold disabled:opacity-50">{loading ? "Creating account…" : "Create Account"}</button></form>}
